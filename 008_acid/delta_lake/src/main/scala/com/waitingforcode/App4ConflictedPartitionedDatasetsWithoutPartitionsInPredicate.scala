...
object App4ConflictedPartitionedDatasetsWithoutPartitionsInPredicate {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // Updates on different partitions but the partition is not part of the 
    // query predicate
    new Thread(() => {
      println(s"Starting 'id = 5 '")
      deltaTable.updateExpr("id = 5", Map("multiplication_result_x2" -> "-1"))
      deltaTable.updateExpr("id = 5", Map("multiplication_result_x2" -> "-1"))
      deltaTable.updateExpr("id = 5", Map("multiplication_result_x2" -> "-1"))
      barrier.countDown()
    }).start()
    new Thread(() => {
      println(s"Starting 'id = 4'")
      deltaTable.updateExpr("id = 4", Map("multiplication_result_x2" -> "-100"))
      deltaTable.updateExpr("id = 4", Map("multiplication_result_x2" -> "-100"))
      deltaTable.updateExpr("id = 4", Map("multiplication_result_x2" -> "-100"))
      barrier.countDown()
    }).start()
